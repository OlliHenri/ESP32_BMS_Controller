      ...
      if (getLocalTime(&timeinfo, 1))
      {
        ...
        if (hal.GetVSPIMutex())
        {
          ...
          if (file && mysettings.loggingEnabled)
          {
            ...
            sprintf(dataMessage, "%04u-%02u-%02u %02u:%02u:%02u,", timeinfo.tm_year, timeinfo.tm_mon, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
            file.print(dataMessage);
            file.print(hal.LastPCF8574AValue(), BIN);
            file.print(',');
...
void IRAM_ATTR PCF8574AInterrupt()
{
...
// Triggered when PCF8574B INT pin goes LOW
void IRAM_ATTR PCF8574BInterrupt()
{
...
void setup()
{
  ...
  else
  {
    ...
    // listDir(LITTLEFS, "/", 0);
    ESP_LOGD(TAG, "Olli LittleFS was mounted.....");
  }
  ...
  SERIAL_DATA.begin(COMMS_BAUD_RATE, SERIAL_8N1, RXD2, TXD2); // Serial for comms to modules, was 2,32
